...
const HomeContainer: React.FC<IHomeContainer> = ({ isExistingMember }) => {
    ...
    const onSignup =async () => {
        const res = await axios.post("/api/users", { name:"Rajasekar"});
        console.log(res);
    }
    return (
        <section className="flex">
            ...
            <div className="flex-[40%] text-center text-black font-[Arial] text-[14px]">
                ...
                <div className="mt-[20px] pb-10">
                    ...
                    {!isExistingMember ? <input className="bg-[red] border border-black border-solid w-[70px] h-[30px] font-600 text-[17px] text-white" type="button" value={"Join"} onClick={onSignup} /> : ''}
                </div>
            </div>
        </section >
    )
